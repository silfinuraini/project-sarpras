...
<body>
    ...
    <p>Dengan ini kami sampaikan laporan pengadaan sebagai berikut:</p>
    ...
    <table class="table">
        ...
        <tbody>
            @foreach ($pengadaan as $index => $p)
                <tr>
                    <td style="font-size: 13px; text-align: center">{{ $index + 1 }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $p->kode }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $p->pegawai->nama ?? '' }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $p->jumlah_item ?? '' }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $p->perihal ?? '' }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $p->sifat ?? '' }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $p->status ?? '' }}</td>
                    <td style="font-size: 13px; text-align: center">{{ $p->created_at->format('d-m-Y') ?? '' }}</td>
